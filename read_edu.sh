...
#
[[ $IFS == $IFS ]] && echo "IFS has default value"
ifs_test() {
    # Вы можете безболезненно переопределять любые переменные окружения
    # в своих функциях, так как все они используют только копии
    # этих переменных.
    unset IFS
}
ifs_test
